...
- `src/commands/` — `projectCommands`, `groupCommands`, `aiCommands`, `gitCommands` + `gitContext.ts`; handler modules live in subfolders `gitHandlers/` (incl. `newVersionHandlers.ts` — new-version-tab backend), `projectHandlers/`, `taskHandlers/`, `todoHandlers/`
- `src/ai/` — `aiClient.ts` (shared OpenAI-compatible client: `projectAtlas.ai.*` config + chat with retry/cancel), `aiCommitService.ts` (commit message generation), `newVersionNotesService.ts` (new-version changelog generation + version bump suggestion), `aiConfigMigration.ts` (one-time `gitAtlas.aiCommit.*` → `projectAtlas.ai.*` migration, plus `projectAtlas.ai.releasePrompt` → `newVersionPrompt` migration with its own `projectAtlas.ai.promptMigrated` flag), `thinkingProviders.ts` (thinking-model support)
...
- `CommandType` / `EventType` unions in `protocol.ts` are git-centric but the router widens `command`/`event` to `string` so project/task/todo command and event names pass through unchanged. `CommandType` also carries the new-version command group (`getNewVersionContext` … `updateNewVersionPrompt`, handled in `newVersionHandlers.ts`), and `EventType` includes `switchTab` (commit-panel tab switching, e.g. `git-atlas.newVersion` → newVersion tab).
...
- The commit panel has **three tabs** (commit / stash / newVersion). New-version-tab state lives in `webview/src/shared/store/new-version-store.ts`, separate from `commit-store`. (A fourth "release" tab for remote Release publishing is planned in `.docs/release-publish.md` but not yet implemented.)
